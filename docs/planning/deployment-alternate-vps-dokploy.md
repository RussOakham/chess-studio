# Alternate deployment: VPS, Docker, and Dokploy

## Status

This document describes a **self-hosted / alternate** deployment option: a **private VPS** with **Docker**, **Dokploy** (or similar), **Nginx**, and optionally **Terraform** for DNS (e.g. Route 53).

**It is not the current production plan.** The active approach is **[`vercel-deployment-plan.md`](./vercel-deployment-plan.md)** (Next.js on Vercel + Convex Cloud).

**When to consider this path:** Vercel limits or pricing, compliance needs, desire for full control over the runtime, or multi-service container orchestration on your own infrastructure.

**Repo reality (chess-studio today):** The application is **Next.js** in `apps/web` plus **Convex** for data and auth. There is **no** separate `apps/api` service in the monorepo. Sections below that reference a **web + API** container split reflect an **earlier design**; a future self-hosted deploy would most likely **containerize `apps/web` only** (and still use Convex Cloud unless you migrate the backend).

## Overview

This section outlines deployment on a **private VPS**, managed with **Dokploy** (or equivalent) and **Docker Compose**.
